...
class MAB():
    ...
    # Initialize alpha, gamma, self.states, actions, rewards, and Q-values
    def __init__(self, paramsML, paramsMH):
        ...
        self.memory     = {}
        self.memory["arms_played"] = []
        self.memory["mean_reward"] = []
        self.memory["rewards_hist"]= []
        self.memory["value_func"]  = [] 

        self.Qvalues = np.zeros(shape=(self.statesQ,len(self.actions))) #self.state,actions
        self.visitas = np.zeros(shape=(self.statesQ,len(self.actions))) #self.state,actions
    ...
    def updateMAB(self):
        sum_rw = np.zeros(len(self.actions))
        cont_rw = np.zeros(len(self.actions))
        iters = len(self.memory["arms_played"])

        for i in range(iters):
            j = self.memory["arms_played"][i]
            sum_rw[j] += self.memory["rewards_hist"][i]
            cont_rw[j] += 1
        mean = np.divide(sum_rw, cont_rw, out=np.zeros_like(sum_rw), where=cont_rw!=0) # Si cont_rw == 0, devuelve 0
        l = np.full(len(self.actions), np.log(iters))
        div = np.true_divide(l, cont_rw, out=np.zeros_like(l), where=cont_rw!=0)
        #div[div == np.NaN] = 100
        mean = mean + self.epsilon * np.sqrt(div)
        self.memory["mean_reward"].append(mean)
        return mean
    ...
    def updateQtable(self,metric,action,newAction,state,newState,iter):
        ...
        self.memory["rewards_hist"].append(Reward)
        self.memory["arms_played"].append(action)
        
        Qnuevo = self.updateMAB()
        
        self.actualizar_Visitas(action) #Actuzación de visitas
        self.Qvalues[self.state][action]= Qnuevo[action]
        self.memory["value_func"].append(Qnuevo)
